...
export default function CuttingOrdersModule({ token, onNavigate }) {
  ...
  const [prog, setProg] = useState({ input_consumed: '', output_qty: '', waste_qty: '', note: '', roll_id: '', input_uom: '' });
  ...
  const onPickMaterial = async (id) => {
    ...
    if (!id) return;
    try {
      setRolls(await cuttingApi('GET', `/rolls?material_id=${id}`, token));
    } catch {
      setRolls([]);
    }
  };
  ...
  const openDetail = async (id) => {
    ...
    setProg({ input_consumed: '', output_qty: '', waste_qty: '', note: '', roll_id: '', input_uom: '' });
    try {
      setDetail(await cuttingApi('GET', `/orders/${id}`, token));
    } catch (e) {
  ...
  const submitProgress = async (e) => {
    ...
    setActing(true);
    try {
      await cuttingApi('POST', `/orders/${detail.id}/progress`, token, {
      ...
      toast.error(e2.message, { duration: 8000 });
    } finally {
      setActing(false);
    }
  };
      ...
      {showCreate && (
        <Modal title="Buat Order Cutting" size="xl" onClose={() => setShowCreate(false)}>
          <form onSubmit={submitCreate} className="space-y-4" data-testid="cutting-create-form">
            ...
            {rolls.length > 0 && (
              <Field label="Roll Fisik (opsional)" hint="Centang roll yang akan dipotong agar sisa roll ikut tercatat.">
                <div className="max-h-32 overflow-y-auto rounded-lg border border-[var(--glass-border)] divide-y divide-[var(--glass-border)]">
                  {rolls.map((r) => {
                    ...
                    return (
                      <label key={r.id} className="flex items-center gap-2 px-3 py-2 text-xs cursor-pointer hover:bg-[var(--nav-pill-active)]/40">
                        ...
                        <span className="text-muted-foreground">
                          sisa {fmtNum(r.uom === 'kg' ? r.remaining_kg : r.remaining_m, 2)} {r.uom} · {r.color || '-'}
                        </span>
                      </label>
                    );
                  })}
            ...
            {detail.status === 'in_progress' && (
              <GlassCard className="p-4">
                ...
                <p className="text-xs text-muted-foreground mb-3">
                  Setiap input langsung memotong stok kain dan menambah stok potongan di gudang.
                </p>
                <form onSubmit={submitProgress} className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end"
                  ...
                  {(detail.roll_ids || []).length > 0 && (
                    <div className="sm:col-span-2">
                      <Field label="Roll yang dipotong (opsional)">
                        <select value={prog.roll_id}
                          onChange={(e) => setProg((p) => ({ ...p, roll_id: e.target.value }))}
                          className={inputCls} data-testid="cutting-progress-roll">
                          <option value="">— tidak spesifik —</option>
                          {(detail.roll_ids || []).map((r) => (
                            <option key={r.roll_id} value={r.roll_id}>
                              {r.roll_no} (sisa {fmtNum(r.remaining, 2)} {r.uom})
                            </option>
                          ))}
                        </select>
                      </Field>
